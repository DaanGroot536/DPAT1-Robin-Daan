    ...
    public class HelpModeState : ModeState
    {
        ...
        public override void Print()
        {
            ...
            for (int y = 0; y < puzzleView.Puzzle.FileInfo.Size; y++)
            {
                ...
                {
                    PrintRow(leafs, y, z, cellWidth);
                }

                PrintHorizontalDivider(puzzleView.Puzzle.FileInfo.Size, cellWidth);
            }

            Console.WriteLine("\nHulpgetallen stand - gebruik de spatiebalk om te wisselen");
        }

        private void PrintRow(List<Cell> leafs, int y, int z, int cellWidth)
        {
            for (int x = 0; x < puzzleView.Puzzle.FileInfo.Size; x++)
            {
                ...
                {
                    SetConsoleColors(cell, i, z);
                    PrintCellValue(cell, i, cellWidth);
                    Console.ResetColor();
                }
                Console.Write("|");
            }
        ...
        private void SetConsoleColors(Cell cell, int i, int z)
        {
        ...
        private void PrintCellValue(Cell cell, int i, int cellWidth)
        {
            if (cell != null)
            {
                if (cell.EnteredValue > 0)
        ...
        private void PrintHorizontalDivider(int size, int cellWidth)
        {
            for (int i = 0; i < size * (cellWidth * puzzleView.Puzzle.FileInfo.ClusterWidth + 1) - 1; i++)
            {
                Console.Write("-");
            }
            Console.WriteLine();
